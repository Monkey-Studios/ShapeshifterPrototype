...
public class SightSpecial : MonoBehaviour
{
    ...
    private void Start()
    {
        player = GameObject.FindGameObjectWithTag("Invisible");
    }
    ...
    void Update()
    {
        ...
        if (distance < radius)
        {
            ...
            if (Physics.Raycast(transform.position, GetPlayerPos, out hitObject, radius) && hitObject.transform.tag == "Invisible")
            {
                GetComponent<TargetingSpecial>().enabled = true;
            }
            else
            {
                GetComponent<TargetingSpecial>().enabled = false;
            }
        }
        else
        {
